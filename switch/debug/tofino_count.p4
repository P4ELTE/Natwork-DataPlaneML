...
#define COUNTING_REGISTER(NAME) \
        Register<count_t, bit<8>>(1, 0) NAME; \
        RegisterAction<count_t, bit<8>, count_t>(NAME) NAME ## _action = { \
            void apply(inout count_t reg_val, out count_t return_value) { \
                reg_val = reg_val + 1; return_value = reg_val; \
            } \
        };

    COUNTING_REGISTER(count_a)
    COUNTING_REGISTER(count_b)

    apply {
        @atomic {
            hdr.ipv4.srcAddr = count_a_action.execute(0);
            hdr.ipv4.dstAddr = count_b_action.execute(0);
        }
